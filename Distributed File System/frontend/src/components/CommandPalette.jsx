...
import {
  Users, Plus, LogIn, Settings, Sun, Moon, LogOut, PanelLeft,
} from "lucide-react";
import Kbd from "./Kbd";
...
export default function CommandPalette({
  open, onOpenChange, groups, isDark,
  onOpenGroup, onNewGroup, onJoin, onSettings, onToggleTheme, onToggleSidebar, onSignOut,
}) {
  ...
  return (
    ...
    >
      ...
      />
      <Command.List className="max-h-80 overflow-y-auto p-2 [&::-webkit-scrollbar]:w-1.5 [&::-webkit-scrollbar-thumb]:rounded-full [&::-webkit-scrollbar-thumb]:bg-gray-300 dark:[&::-webkit-scrollbar-thumb]:bg-neutral-700">
        ...
        <Command.Group heading="App">
          ...
          <Item onSelect={run(onToggleTheme)}   icon={isDark ? <Sun size={15} /> : <Moon size={15} />}>{isDark ? "Light mode" : "Dark mode"}</Item>
  ...
  return (
    <Command.Item
      ...
      className="flex items-center gap-2.5 px-3 py-2 rounded-lg text-sm cursor-pointer text-gray-700 dark:text-neutral-200
                 aria-selected:bg-blue-100/70 dark:aria-selected:bg-[#FF6363]/15 aria-selected:text-blue-700 dark:aria-selected:text-[#FF6363]"
    >
